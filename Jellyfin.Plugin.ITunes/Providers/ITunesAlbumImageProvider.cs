...
public class ITunesAlbumImageProvider : IRemoteImageProvider, IHasOrder
{
    ...
    private async Task<IEnumerable<RemoteImageInfo>> GetImagesInternal(string url, CancellationToken cancellationToken)
    {
        ...
        {
            foreach (AlbumResult album in iTunesAlbumDto.Results)
            {
                if (!string.IsNullOrEmpty(album.ArtworkUrl100))
                {
                    ...
                    var image1400 = album.ArtworkUrl100.Replace("100x100bb", "1400x1400bb", StringComparison.OrdinalIgnoreCase);
